...
/**
 * @author ：张音乐
 * @date ：Created in 2021/8/10 下午7:12
 * @description：覆盖默认DatabaseMetaData
...
public class DatabaseMetaData implements java.sql.DatabaseMetaData {
    ...
    protected DatabaseMetaData(JdbcConnection connToSet, String databaseToSet, ResultSetFactory resultSetFactory) {
        ...
        this.databaseTerm = this.conn.getPropertySet().<PropertyDefinitions.DatabaseTerm>getEnumProperty(PropertyKey.databaseTerm);
    ...
    protected void convertToJdbcFunctionList(ResultSet proceduresRs, List<ComparableWrapper<String, Row>> procedureRows, Field[] fields) throws SQLException {
        while (proceduresRs.next()) {
            ...
            if (fields != null && fields.length == 9) {
                ...
                rowData = new byte[9][];
                rowData[0] = this.databaseTerm.getValue() == PropertyDefinitions.DatabaseTerm.SCHEMA ? s2b("def") : s2b(procDb);   // PROCEDURE_CAT
                rowData[1] = this.databaseTerm.getValue() == PropertyDefinitions.DatabaseTerm.SCHEMA ? s2b(procDb) : null;         // PROCEDURE_SCHEM
    ...
    private Row convertTypeDescriptorToProcedureRow(byte[] procNameAsBytes, byte[] procCatAsBytes, String paramName, boolean isOutParam, boolean isInParam,
        ...
        row[0] = this.databaseTerm.getValue() == PropertyDefinitions.DatabaseTerm.SCHEMA ? s2b("def") : procCatAsBytes;                                // PROCEDURE_CAT
        row[1] = this.databaseTerm.getValue() == PropertyDefinitions.DatabaseTerm.SCHEMA ? procCatAsBytes : null;                                      // PROCEDURE_SCHEM
    ...
    public ResultSet extractForeignKeyFromCreateTable(String dbName, String tableName) throws SQLException {
        ...
        if (tableName != null) {
            ...
            try {
                rs = this.databaseTerm.getValue() == PropertyDefinitions.DatabaseTerm.SCHEMA ? getTables(null, dbName, null, new String[] { "TABLE" })
    ...
    protected Field[] createColumnsFields() {
        ...
        fields[24] = new Field("", "EXTRA", this.metadataCollationIndex, this.metadataEncoding, MysqlType.CHAR, 255);
        return fields;
    }
    ...
    public java.sql.ResultSet getCrossReference(final String primaryCatalog, final String primarySchema, final String primaryTable, final String foreignCatalog,
        ...
        final boolean dbMapsToSchema = DatabaseMetaData.this.databaseTerm.getValue() == PropertyDefinitions.DatabaseTerm.SCHEMA;
                    ...
                    while (st.hasMoreTokens()) {
                        ...
                        try {
                            ...
                            while (columnResults.next()) {
                                byte[][] tuple = new byte[8][];
                                tuple[0] = this.databaseTerm.getValue() == PropertyDefinitions.DatabaseTerm.SCHEMA ? s2b("def") : s2b(db);  // PKTABLE_CAT
                                tuple[1] = this.databaseTerm.getValue() == PropertyDefinitions.DatabaseTerm.SCHEMA ? s2b(db) : null;        // PKTABLE_SCHEM
                                tuple[2] = s2b(table);
